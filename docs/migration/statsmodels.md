...
| ARIMA / SARIMAX estimation, state-space | ⛔ **not written** — [`decisions/0134`](../decisions/0134-arima-and-state-space-are-not-written-and-var-is-the-one-that-could-be.md): `statsmodels`' own optimisers disagree at `1e-4` on one series, so no corpus can pin them; `Cortex.TimeSeries`' ARIMA fits a pure autoregression under an ARIMA name |
| `VAR(y).fit(p)`: vector autoregression with its table | **native**: [`VectorAutoregression`](../reference/stats-timeseries/var.md) in `Lodestar.Stats.TimeSeries`, at statsmodels parity — least squares on the stacked lags, the residual covariances and the four information criteria. Nothing in .NET estimated one; [`decisions/0134`](../decisions/0134-arima-and-state-space-are-not-written-and-var-is-the-one-that-could-be.md), [#786](https://github.com/CyrilB1531/lodestar/issues/786) |
