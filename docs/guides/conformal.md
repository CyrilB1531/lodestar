...
double q = SplitConformal.Quantile(scores, 0.25, ConformalQuantileRule.MapieClassification);   // 0.5
...
[`ConformalQuantileRule.MapieClassification`](../reference/conformal/prediction/conformalquantilerule.md)
is the rule MAPIE's classifier reads, which can sit one rank above the default ceiling rule; without
it the sets can differ from MAPIE's by that rank.
...
## A positive target: the gamma score

Prices, durations and counts are errors that grow with the value: a 10 % miss on 1,000 is not a 10 %
miss on 10. `GammaScores` scores the relative error `(y − ŷ) / ŷ`, signed, and `GammaInterval` turns
it into `[ŷ(1 + q_low), ŷ(1 + q_up)]` — proportional to the prediction, and free to be wider on the
side the errors fall. It takes the scores rather than a quantile, because each side reads its own —
[`SplitConformal.GammaScores`](../reference/conformal/prediction/splitconformal-gammascores.md)
and [`SplitConformal.GammaInterval`](../reference/conformal/prediction/splitconformal-gammainterval.md).
...
Every target and every prediction must be strictly positive; zero or below is refused, as MAPIE
refuses it.

## Without a calibration set: CV+ and Jackknife+

Split conformal spends data the model never sees. When there is too little to spare,
[`CrossConformal`](../reference/conformal/prediction/crossconformal.md) scores every training
sample with a model fitted without it — the model of the
fold it sat out (CV+), the model fitted without that one sample (Jackknife+), or the bootstrap
models it was out of bag for — and builds a new point's interval from every model's prediction
there, through
[`CrossConformal.Interval`](../reference/conformal/prediction/crossconformal-interval.md). As
everywhere here, the models are yours: you hand over their predictions and which samples each one
was fitted without.

```csharp
using Lodestar.Conformal;

int[] folds = [0, 0, 1, 1, 2, 2];                           // model m was fitted without fold m
double[] yTrue = [10.2, 11.5, 9.8, 12.6, 11.1, 10.4];
double[] outOfFold = [10.7, 10.5, 10.0, 11.8, 11.4, 11.0];  // each sample by its fold's model
double[] atTest = [10.0, 11.0, 12.0];                       // the three models at a new point
...
**The guarantee is weaker: `1 − 2α`**, not `1 − α` (Barber et al., 2021), though coverage near
`1 − α` is what is usually seen. For the jackknife-after-bootstrap, pass the out-of-bag mask
instead of the folds, and `OutOfSample` to aggregate each sample's out-of-bag predictions.
...
Every value in this guide is replayed from frozen corpora generated by MAPIE 1.5.0 —
`SplitConformalRegressor` and `SplitConformalClassifier` at `conformity_score="lac"`, both
`prefit`, the gamma score, and `CrossConformalRegressor` and `JackknifeAfterBootstrapRegressor`. The one deliberate divergence is the `k > n` edge above; prediction sets match at
`ConformalQuantileRule.MapieClassification`.
