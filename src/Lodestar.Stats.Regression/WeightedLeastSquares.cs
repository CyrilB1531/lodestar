...
public static class WeightedLeastSquares
{
    ...
    /// <exception cref="ArgumentException"><paramref name="design"/> is not a whole number of rows, <paramref name="response"/> or <paramref name="weights"/> has a different length, <paramref name="options"/> sets an option its covariance type does not read or asks for <see cref="CovarianceType.Hac"/> without lags or <see cref="CovarianceType.Cluster"/> without labels, no residual degrees of freedom are left, fewer rows carry a positive weight than there are parameters, or a column of the weighted design, intercept included, is collinear with the columns before it.</exception>
    /// <remarks>
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="featureCount"/> is not positive, or a weight is negative, <c>NaN</c> or infinite.</exception>
    /// <exception cref="ArgumentException"><paramref name="design"/> is not a whole number of rows, <paramref name="response"/>, <paramref name="weights"/> or <paramref name="clusters"/> has a different length, <paramref name="clusters"/> names fewer than two clusters, <paramref name="options"/> asks for another covariance, no residual degrees of freedom are left, fewer rows carry a positive weight than there are parameters, or a column of the weighted design, intercept included, is collinear with the columns before it.</exception>
    /// <remarks>
