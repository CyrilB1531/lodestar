...
/// <summary>How <c>OrdinaryLeastSquares</c> estimates the covariance of its estimates.</summary>
/// <remarks>
...
/// the F, and <c>OlsSummary.CovarianceType</c> says which was used. Decision 0004 has why
...
public enum CovarianceType
{
    ...
    /// <summary>Newey and West's estimator for errors correlated along the row order: the scores of rows up to <c>OlsOptions.HacLags</c> apart enter with Bartlett weights.</summary>
    /// <remarks>
    /// Rows are read as a time series in the order given. <c>statsmodels</c>' <c>cov_type="HAC"</c> with its default
    /// kernel; <c>OlsOptions.SmallSampleCorrection</c> scales by <c>n / (n - k)</c> and is off by default, as there.
    /// </remarks>
    Hac = 5,

    /// <summary>One-way cluster-robust: errors free to correlate inside a cluster and independent across clusters.</summary>
    /// <remarks>
    /// Needs the overload taking one cluster label per row. <c>OlsOptions.SmallSampleCorrection</c> scales by
