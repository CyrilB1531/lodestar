...
internal static class ShortNeedleWindows
{
    ...
    /// the fraction, so the maximum it returns is the one scoring every window would have found.
    /// </remarks>
    private struct BestWindow
    {
        private int _lcs;
        private int _total;
        ...
        {
            if (_total == 0 || lcs * _total > _lcs * total)
            {
                _lcs = lcs;
                _total = total;
            }
        ...
        public readonly bool CouldImprove(int lcsCeiling, int total) =>
            _total == 0 || lcsCeiling * _total > _lcs * total;
