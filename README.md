...
| `Lodestar.Stats` | `Accord.Statistics` (archived, no longer maintained) | No case found where `Accord` and `scipy` (and therefore `Lodestar.Stats`) disagree; faster on the t-test, Mann-Whitney and, since #710, the chi-square table (66.5 ns against 121.7 ns) — see [`docs/guides/hypothesis-testing.md`](docs/guides/hypothesis-testing.md#no-incumbent-to-compare-against) |
