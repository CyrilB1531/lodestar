...
workflow file therefore paid all four: on #1072, which changed `ci.yml` and nothing
else, 6 minutes of build and test, 2.1 of sample, 1.1 of oracles, and a Windows job
still running after the required checks had gone green.

Two classes answer ``true`` here:

* **Markdown only**, which is `docs_only.py`'s answer — the reduced path #857 added.
* **Workflow only**, meaning every path is a workflow-ish file **other than
  `.github/workflows/ci.yml`**: nothing in the pull-request pipeline reads
  `release.yml`, `release-nuget-org.yml`, `bench-nightly.yml`, `bench-ondemand.yml`,
  `classify-pull-request.yml`, `wiki.yml` or `dependabot.yml`, so a build, a test run
  and a platform check cannot say anything about a change to them.
...
# Spelled out rather than globbed, so a new kind of file under `.github/` has to be
# considered on purpose instead of inheriting the reduced path.
CONFIGURED = frozenset({".github/dependabot.yml", ".github/dependabot.yaml"})
