...
public sealed class DutchSnowballStemmerOracleTests
{
    private static readonly OracleFile<PorterCase> Corpus = OracleCorpus.Load<PorterCase>("snowball_nl.json");
    ...
    public void Stem_matches_nltk()
    {
        OracleAsserts.ExactString(Corpus.Cases,
            c => c.Stem,
            c => DutchSnowballStemmer.Stem(c.Word),
            c => $"[#{c.Id}] \"{c.Word}\"");
    }
    ...
    {
        Assert.Throws<ArgumentNullException>(() => DutchSnowballStemmer.Stem(null!));
    }
}
