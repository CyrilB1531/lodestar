...
public sealed class SwedishSnowballStemmerOracleTests
{
    private static readonly OracleFile<PorterCase> Corpus = OracleCorpus.Load<PorterCase>("snowball_sv.json");
    ...
    public void Stem_matches_nltk()
    {
        OracleAsserts.ExactString(Corpus.Cases,
            c => c.Stem,
            c => SwedishSnowballStemmer.Stem(c.Word),
            c => $"[#{c.Id}] \"{c.Word}\"");
    }
    ...
    {
        Assert.Throws<ArgumentNullException>(() => SwedishSnowballStemmer.Stem(null!));
    }
}
