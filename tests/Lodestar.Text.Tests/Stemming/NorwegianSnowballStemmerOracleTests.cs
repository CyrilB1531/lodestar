...
public sealed class NorwegianSnowballStemmerOracleTests
{
    private static readonly OracleFile<PorterCase> Corpus = OracleCorpus.Load<PorterCase>("snowball_no.json");
    ...
    public void Stem_matches_nltk()
    {
        OracleAsserts.ExactString(Corpus.Cases,
            c => c.Stem,
            c => NorwegianSnowballStemmer.Stem(c.Word),
            c => $"[#{c.Id}] \"{c.Word}\"");
    }
    ...
    {
        Assert.Throws<ArgumentNullException>(() => NorwegianSnowballStemmer.Stem(null!));
    }
}
