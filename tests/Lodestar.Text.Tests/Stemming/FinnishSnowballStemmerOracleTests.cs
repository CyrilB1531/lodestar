...
public sealed class FinnishSnowballStemmerOracleTests
{
    private static readonly OracleFile<PorterCase> Corpus = OracleCorpus.Load<PorterCase>("snowball_fi.json");
    ...
    public void Stem_matches_nltk()
    {
        OracleAsserts.ExactString(Corpus.Cases,
            c => c.Stem,
            c => FinnishSnowballStemmer.Stem(c.Word),
            c => $"[#{c.Id}] \"{c.Word}\"");
    }
    ...
    {
        Assert.Throws<ArgumentNullException>(() => FinnishSnowballStemmer.Stem(null!));
    }
}
