...
public sealed class NetStandardAssemblyGuardTests
{
    ...
    {
        AssertNetStandard(typeof(SerialCorrelation));
    }

    /// <summary>The same guarantee for Lodestar.Stats, which carries the tail and the quantile.</summary>
    [Fact]
    ...
    /// <summary>The same guarantee for Lodestar.Stats.Regression, which fits the unit-root regressions.</summary>
    [Fact]
    public void Suite_runs_against_the_netstandard2_0_build_of_Lodestar_Stats_Regression()
    {
        AssertNetStandard(typeof(Lodestar.Stats.Regression.OrdinaryLeastSquares));
    }

    /// <summary>The same guarantee for Lodestar.Decomposition, which carries the regressions' QR.</summary>
    [Fact]
