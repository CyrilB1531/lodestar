...
/// <summary>How strongly the instruments predict the endogenous regressor.</summary>
internal static class IvFirstStageSample
{
    public static void Run()
    {
        Console.WriteLine("IV first stage (Lodestar.Stats.Regression)");
        ...
        Console.WriteLine($"  R² / partial / Shea : {Inv.F4(first.RSquared)} / {Inv.F4(first.PartialRSquared)} / {Inv.F4(first.SheaRSquared)}");
        Console.WriteLine($"  instruments test    : {Inv.F1(first.InstrumentTest.Statistic)}");
        Console.WriteLine();
    }
}
