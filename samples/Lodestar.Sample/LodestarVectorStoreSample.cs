...
internal static class LodestarVectorStoreSample
{
    public static async Task RunAsync()
    {
        ...
        VectorStoreCollection<string, VectorDataCorpus.Note> notes =
            store.GetCollection<string, VectorDataCorpus.Note>("notes");
        ...
        Console.WriteLine($"  store knows it    : {await store.CollectionExistsAsync("notes").ConfigureAwait(false)}");
        ...
        await store.EnsureCollectionDeletedAsync("notes").ConfigureAwait(false);
        Console.WriteLine($"  deleted           : {!await store.CollectionExistsAsync("notes").ConfigureAwait(false)}");
        Console.WriteLine();
    }
}
